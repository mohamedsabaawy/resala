...
<div class="card">
    <div class="card-header d-flex">
        <h3 class="card-title ">الجنسيات</h3>
    </div>
    <!-- /.card-header -->
    <div class="card-body">
        <div class="row">
        ...
        <table id="example1" class="table table-bordered table-striped">
            ...
            @forelse($nationalities as $nationality)
                <tr class="{{$nationality->deleted_at ? 'bg-gradient-gray':''}}">
                    <td>{{$nationality->id}}</td>
                    <td >{{$nationality->name}}</td>
                    <td>
                        <div class="btn-group">
                            <button type="button" class="btn btn-danger" data-toggle="modal" data-target="#delete-model" wire:click="show({{$nationality->id}})" ><i class="fa fa-trash"></i>مسح</button>
                            <button type="button" class="btn btn-warning" data-toggle="modal" data-target="#create-model" wire:click="show({{$nationality->id}})"><i class="fa fa-edit"></i>تعديل</button>
                        </div>
                    </td>
                </tr>
            @empty
    ...
    <!-- start create nationality model -->
{{--    wire:ignore.self--}}
<!-- start nationality model -->
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <div class="modal-header">
                    <h4 class="modal-title">{{$isUpdate ? "تعديل جنسية" :"انشاء جنسية جديد"}}</h4>
                ...
                <div class="modal-body">
                    ...
                    <div class="form-group">
                        <label for="exampleInputEmail1">اسم الجنسية</label>
                        <input type="text" class="form-control @error('name') is-invalid @enderror" wire:keydown.enter="{{$isUpdate ? "update()" : "save()"}}" wire:model="name" placeholder="ادخل اسم الجنسية">
