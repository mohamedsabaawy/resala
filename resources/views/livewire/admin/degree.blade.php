@section('title','إدارة درجات التطوع')
<div class="card">
    <div class="card-header d-flex">
        <h3 class="card-title ">درجات التطوع</h3>
    </div>
    <!-- /.card-header -->
    <div class="card-body">
        <div class="row">
        ...
        <table id="example1" class="table table-bordered table-striped">
            ...
            @forelse($degrees as $degree)
                <tr class="{{$degree->deleted_at ? 'bg-gradient-gray':''}}">
                    <td>{{$degree->id}}</td>
                    <td >{{$degree->name}}</td>
                    <td>
                        <div class="btn-group">
                            <button type="button" class="btn btn-danger" data-toggle="modal" data-target="#delete-model" wire:click="show({{$degree->id}})" ><i class="fa fa-trash"></i>مسح</button>
                            <button type="button" class="btn btn-warning" data-toggle="modal" data-target="#create-model" wire:click="show({{$degree->id}})"><i class="fa fa-edit"></i>تعديل</button>
                        </div>
                    </td>
                </tr>
            @empty
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <div class="modal-header">
                    <h4 class="modal-title">{{$isUpdate ? "تعديل درجة تطوع" :"انشاء درجة تطوع جديد"}}</h4>
                ...
                <div class="modal-body">
                    ...
                    <div class="form-group">
                        <label for="exampleInputEmail1">اسم الدرجة تطوع</label>
                        <input type="text" class="form-control @error('name') is-invalid @enderror" wire:keydown.enter="{{$isUpdate ? "update()" : "save()"}}" wire:model="name" placeholder="ادخل اسم الدرجة تطوع">
